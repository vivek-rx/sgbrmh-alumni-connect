...
export default function Jobs() {
  ...
  const handleApply = async (jobId: string, applicationLink: string | null) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("job_applications")
        .insert([{ job_id: jobId, alumni_id: user?.id, status: "applied" }]);
      if (error) {
        if (error.code === "23505") {
  ...
  const handleDeleteJob = async (jobId: string) => {
    if (!profile || profile.role !== "admin") {
      toast.error("Only admins can delete jobs");
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-red-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Only verified users can view jobs</p>
        </div>
      </div>
    );
  }
              ...
              filteredJobs.map((job, index) => (
                ...
                >
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <span
                          className={`px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap ${getJobTypeColor(job.type)}`}
                        >
                          {job.type
                            .split("-")
                            .map(
                  ...
                  <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-100">
                    {job.has_applied ? (
                      <div className="flex items-center px-6 py-3 bg-green-100 text-green-700 rounded-lg font-medium">
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={() =>
                          handleApply(job.id, job.application_link)
                        }
                        className="flex items-center px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white font-medium rounded-lg hover:from-orange-600 hover:to-red-600 transition-all shadow-md"
                      >
                    ...
                    {profile?.role === 'admin' && (
                      <motion.button
                        ...
                        className="flex items-center px-6 py-3 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-all shadow-md"
